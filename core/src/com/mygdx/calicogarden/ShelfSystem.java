...
public void draw(SpriteBatch batch) {
    ...
    for (int i = 0; i < plants.length; i++) {
        ...
        System.out.println("Drawing plant_" + i + " at x: " + plantBounds[i].x + ", y: " + plantBounds[i].y + 
                           " with width: " + plantBounds[i].width + " and height: " + plantBounds[i].height);
        
        // Draw plants based on plantBounds positions
        
            batch.draw(plants[i].getTexture(), plantBounds[i].x, plantBounds[i].y, plantBounds[i].width * 2f, plantBounds[i].height * 2f);
...
public void loadState() {
    ...
    for (int i = 0; i < plants.length; i++) {
        ...
        System.out.println("Loaded plant_" + i + " x: " + x + ", y: " + y); // Add debug output
    }
}
...
public void resetState() {
    ...
    saveState(); // Reload the default state after resetting
}
